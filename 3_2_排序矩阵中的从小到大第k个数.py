...
'''主要思路：
思路1（时间O(k*(m+n)), 空间O(1)）：
利用类似于小顶堆的方法，将排序矩阵 matrix 化为小顶堆，即 matrix[0][0] 为矩阵中最小元素，
且矩阵每一行递增，每一列也递增，在取出堆顶元素后，将堆顶元素设置为最大值float('inf')，
然后调整矩阵，使其符合小顶堆。如此一来，取出的第 k 个元素即排序矩阵中的从小到大第k个数
关键在于如何调整矩阵：比较inf右边的数和下边的数，小的那个和inf交换，一直下去，
直到inf来到右边界或下边界，此时只需把inf向下移到最底部或右移到最右端，即把inf移到右下角
思路2（时间O(k*min(m,n)), 空间O(min(m,n)）：
如果行数小于等于列数，则列优先遍历，反之行优先遍历，这里解释如何进行列优先遍历（行优先同理）：
k次扫描，每次扫描都把每一行的最小元素进行比较，找出这次比较中的最小值 min，
如果这次找出的元素在第i行第j列，那么第i行下一次比较就从j+1开始 
比如第一次扫描的最小值是0行的0列元素，那么第二次扫描第0行第0列元素就会被排除，第0行从第1列开始比较
思路3（时间O(k*log(max(m,n))), 空间O(m*n）：
通过思路2发现，每筛选出一个最小值，下一次可能会成为最小值的位置位于该最小值的右边和下边，
...
class Solution:
    ...
    def kthSmallest3(self, matrix, k):
        ...
        if rows * cols < k or k < 1:
            return
        minHeap = []
        visited = [[False] * cols for i in range(rows)]  # python创建二维数组不能用[[False] * cols] * rows
