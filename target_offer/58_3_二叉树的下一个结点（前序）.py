__author__ = 'Hk4Fun'
__date__ = '2018/2/23 19:10'

'''题目描述：
给定一个二叉树和其中的一个结点，请找出前序遍历顺序的下一个结点并且返回。
注意，树中的结点不仅包含左右子结点，同时包含指向父结点的指针。
'''
'''主要思路：
思路1：若该结点有左孩子则下一个结点为该左孩子；
       否则若该结点有右孩子则下一个结点为该右孩子；
       既没有左孩子也没有右孩子，则下一个结点为第一个右兄弟（右堂兄弟）。
       如何找到第一个右兄弟（右堂兄弟）：
       向上迭代找到第一个其父亲的右孩子存在且不是自己的那个结点
       退到了根节点仍没找到则返回None
...
class Solution:
    def GetNext1(self, pNode):
        ...
        if pNode.right:
            return pNode.right
        while pNode.parent:
            if pNode.parent.right and pNode.parent.right is not pNode:
