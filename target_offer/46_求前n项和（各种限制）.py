...
思路2：递归实现，利用模零异常捕获0的情况退出递归
思路3: 递归实现，利用字典进行函数的映射，同时注意到非0值作两次非运算返回True, 0作两次非运算返回False
思路4：利用公式，(n+1)n/2，除2用位运算替代，而乘法可用快速乘法实现（原理：乘法分配律）
       快速乘法: a = 2^e0 + 2^e1 + 2^e2....
       那么 a * b  = (2^e0 + 2^e1 + 2^e2+...) * b = b * 2^e0 + b * 2^e1 + b * 2^e2 + ...
                   = (b << e0) + (b << e1) + ....
       可以看到，快速乘法把乘法运算拆开转化为位运算和加法，在大数运算下可能比直接用乘法要快
...
class Solution:
    ...
    def Sum4(self, n):
        ...
        while a:  # 可以复制32次来替代循环，这里就不这么做了
            ...
            # 注意 1 and 2 返回 2，即前面为True就返回后面的值
            # b在这里左移部分应该移出，所以必须截取后面32位，
            # if a & 1:
            #     s += b
