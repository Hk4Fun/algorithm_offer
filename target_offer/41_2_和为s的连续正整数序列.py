...
思路2：双指针问题，当总和小于sum，大指针继续+，否则小指针+。
       实际上和思路1没什么区别，只是代码结构清晰一些。
       但由于没有上限，使得情况发生重复，所以时间效率反而没有思路1好
思路3：(a1+an)*n/2=s, n=an-a1+1  ==>  (an+a1)*(an-a1+1)=2s=i*j(设i>=j), 其中i=an+a1, j=an-a1+1
       由 i=an+a1, j=an-a1+1 解得 an=(i+j-1)/2, a1=(i-j+1)/2 。
       所以只需遍历i和j，但i和j有条件，因为an和a1为正整数，
       所以(i+j)和(i-j)必须为奇数，也就是说i和j必为一奇一偶，所以i>j。
       所以j的上限为sqrt(2s)，而j的下限为2，若为1则i=2s，
       an=a1=s，不合题意。那么如何判断i和j是否一奇一偶呢？(i^j)&1==1时i和j必为一奇一偶
思路4：1）由于我们要找的是和为S的连续正整数序列，因此这个序列是个公差为1的等差数列，
          所以这个序列的中间值代表了平均值的大小。
       ...
       3）由题可知n >= 2，那么n的最大值是多少呢？
          我们让序列从1开始，根据等差数列的求和公式：S = (1 + n) * n / 2，
          得到 n <= sqrt(2s).
'''
