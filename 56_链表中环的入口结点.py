...
'''
'''主要思路：
此题和15_4是一样的，这里提供其他思路：
思路1： 先获取环的长度n，然后在头部设两个指针p1，p2，让p1先走n步，然后两个指针以相同速度向前移动，
...
思路2： 断链法。两个指针，一个在前面before，另一个紧邻着这个指针，在后面after。
...
思路3： 遍历链表，用一个数组存放遍历过的结点指针，则遍历遇见的第一个重复的指针即为入口结点
